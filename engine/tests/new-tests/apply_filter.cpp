...
TYPED_TEST(ApplyFilter, withNull)
{
    ...
    // Then we create a cudf::table_view from the column wrappers we created
    cudf::table_view cudf_table_in_view ({col1, col2, col3});
    // Then using a vector of names and the cudf::table_view we are able to create a BlazingTableView
...
TYPED_TEST(ApplyFilterDates, interatorWithNull)
{
    ...
    cudf::table_view cudf_table_in_view ({timestamp_col, int32_col});
