...
static inline void ReadEstimatedTableRowCount(
	...
	oss << "select ROW_COUNT from \"" << sql.schema
...
static inline std::string MapColumnType(
	const char buffer[], const std::size_t scale) {
...
static inline void PopulateTableInfo(SQLHDBC sqlHdbc,
	...
	if(sqlReturn != SQL_SUCCESS) {
		throw std::runtime_error(
			"SnowFlake: exec direct for information schema");
	}
	...
	while(SQL_SUCCEEDED(sqlReturn = SQLFetch(sqlHStmt))) {
		...
		std::transform(std::cbegin(buffer),
			std::cend(buffer),
			std::begin(buffer),
...
data_handle snowflake_data_provider::get_next(bool open_file) {
	...
	handle.sql_handle.snowflake_sqlhdbc.reset(
		new SQLHSTMT{sqlHStmt}, sqlHStmt_deleter);

	SQLLEN RowCount = 0;
	sqlReturn = SQLRowCount(sqlHStmt, &RowCount);
	if(sqlReturn != SQL_SUCCESS) {
		throw std::runtime_error(
			"SnowFlake: getting row count for query: " + query);
	}
