	...
	for (size_t i = 0; i < aggregation_types.size(); i++){
		...
		if (agg_input_expr == "" && aggregation_types[i] == AggregateKind::COUNT_ALL) { // this is a COUNT(*)
			...
			if (num_columns_to_count == 1 || is_number(agg_input_expr)) {
				aggregation_input = table_view->view().column(get_index(agg_input_expr));
			} else {
				aggregation_input_scope_holder = ral::processor::evaluate_expressions(table_view->view(), {agg_input_expr});
			...
			if(agg_input_expr == "" && aggregation_types[i] == AggregateKind::COUNT_ALL) { // this is a COUNT(*)
				...
				if (is_multi_var) {
					for (size_t i = 0; i < all_indices.size(); ++i) {
						output_name += table_view->column_names().at(all_indices[i]) + ", ";
					}
					output_name = output_name.substr(0, output_name.size() - 2) + ")";
				} else {
					output_name += table_view->column_names().at(get_index(aggregation_input_expressions[i])) + ")";
				}
...
std::unique_ptr<ral::frame::BlazingTable> compute_aggregations_with_groupby(
			...
			if (expression == agg_input_expr){
				...
				if (!got_aggregation_input) {
					if(expression == "" && aggregation_types[i] == AggregateKind::COUNT_ALL ) { // this is COUNT(*). Lets just pick the first column
						...
						std::vector< std::unique_ptr<ral::frame::BlazingColumn> > computed_columns = ral::processor::evaluate_expressions(table_view->view(), {agg_input_expr});
	...
	return std::make_unique<BlazingCudfTable>(std::move(output_table), output_names);
}
