...
#include "detail/CacheDataLocalFile.h"
#include "bmr/BlazingMemoryResource.h"
#include "detail/CacheDataLocalFile.h"

#include "cache_machine/cache_machine_api.h"
...
*/
class ArrowCacheMachine : public CacheMachine {
 public:
  ArrowCacheMachine(std::shared_ptr<Context> context, std::string cache_machine_name,
...
ArrowCacheMachine::ArrowCacheMachine(std::shared_ptr<Context> context,
                                     std::string cache_machine_name, bool log_timeout,
                                     int cache_level_override, bool is_array_access)
    : CacheMachine(context, cache_machine_name, log_timeout, cache_level_override,
                   is_array_access) {
...
ArrowCacheMachine::~ArrowCacheMachine() {}

bool ArrowCacheMachine::addCacheData(std::unique_ptr<ral::cache::CacheData> cache_data,
  ...
  if ((!this->something_added || cache_data->num_rows() > 0) || always_add) {
    ...
    if (type == ral::cache::CacheDataType::CPU) {
      cacheIndex = 1;
    } else {
      cacheIndex = 2;
    }
...
bool ArrowCacheMachine::addToCache(std::unique_ptr<ral::frame::BlazingTable> table,
                                   std::string message_id, bool always_add,
  ...
  if (!this->something_added || table->num_rows() > 0 || always_add) {
    ...
    // WSM TODO do we want to use the backend_dispatcher here too? This is more business
    // logic, not data transformation
    size_t cacheIndex = table->get_execution_backend().id() == ral::execution::backend_id::ARROW ? 1 : 2;
    while (cacheIndex < memory_resources.size()) {
      ...
      if (memory_to_use < this->memory_resources[cacheIndex]->get_memory_limit() ||
        ...
        if (cacheIndex == 1 && table->get_execution_backend().id() == ral::execution::backend_id::ARROW) {
          ...
          arrow_table->ensureOwnership();
          std::unique_ptr<CacheData> cache_data = std::make_unique<ArrowCacheData>(std::move(arrow_table), metadata);
          auto item = std::make_unique<message>(std::move(cache_data), message_id);
          this->waitingCache->put(std::move(item));
          ...
          if (cache_events_logger) {
            ...
            std::string orc_files_path =
                ral::communication::CommunicationData::getInstance()
                    .get_cache_directory();
            auto cache_data = std::make_unique<ArrowCacheDataLocalFile>(std::move(table), orc_files_path, (ctx ? std::to_string(ctx->getContextToken()) : "none"));
...
/**
   ...
   in the ExecutionGraph.
        ...
        This ArrowConcatenatingCacheMachine::pullFromCache method does not guarantee the
   relative order of the messages to be preserved
*/
class ArrowConcatenatingCacheMachine : public ArrowCacheMachine {
 public:
  ArrowConcatenatingCacheMachine(std::shared_ptr<Context> context,
                            std::string cache_machine_name)
      : ArrowCacheMachine(context, cache_machine_name) {}
  ...
  ~ArrowConcatenatingCacheMachine() = default;
...
std::unique_ptr<ral::frame::BlazingTable> ArrowConcatenatingCacheMachine::pullFromCache(
  ...
  if (collected_messages.empty()) {
    ...
    for (size_t i = 0; i < collected_messages.size(); i++) {
      ...
      if (!concat_all && backend.id() == ral::execution::backend_id::ARROW && checkIfConcatenatingStringsWillOverflow(table_views)) {
        std::unique_ptr<CacheData> cache_data = ral::execution::backend_dispatcher(tables_holder.back()->get_execution_backend(), make_cachedata_functor(), std::move(tables_holder.back()));
        ...
        if (cache_events_logger) {
          cache_events_logger->warn(
              ...
              "timestamp_end"_a = cacheEventTimer.end_time(),
              "description"_a =
                  "In ArrowConcatenatingCacheMachine::pullFromCache Concatenating could have "
    ...
    if (concat_all && checkIfConcatenatingStringsWillOverflow(
      ...
      if (cache_events_logger) {
        cache_events_logger->warn(
            ...
            "timestamp_end"_a = cacheEventTimer.end_time(),
            "description"_a =
                "In ArrowConcatenatingCacheMachine::pullFromCache Concatenating will overflow "
                "strings length");
      }
    }
  ...
  if (cache_events_logger) {
    cache_events_logger->trace(
        ...
        "description"_a = "Pull from ArrowConcatenatingCacheMachine");
  }

  return output;
}

std::unique_ptr<ral::cache::CacheData> ArrowConcatenatingCacheMachine::pullCacheData() {
  CodeTimer cacheEventTimer;
  cacheEventTimer.start();
  ...
  if (cache_events_logger) {
    cache_events_logger->trace(
        ...
        "timestamp_begin"_a = cacheEventTimer.start_time(),
        "timestamp_end"_a = cacheEventTimer.end_time(),
        "description"_a = "Pull cache data from ArrowConcatenatingCacheMachine");
  }
...
std::shared_ptr<CacheMachine> make_arrow_single_machine(
    ...
    int cache_level_override, bool is_array_access) {
  return std::shared_ptr<CacheMachine>(new ArrowCacheMachine(
      context, cache_machine_name, log_timeout, cache_level_override, is_array_access));
}


std::shared_ptr<CacheMachine> make_arrow_concatenating_machine(
 ...
 return std::shared_ptr<CacheMachine>(
