...
*/
class CudfCacheMachine : public CacheMachine {
 public:
  CudfCacheMachine(std::shared_ptr<Context> context, std::string cache_machine_name,
...
CudfCacheMachine::CudfCacheMachine(std::shared_ptr<Context> context,
...
CudfCacheMachine::~CudfCacheMachine() {}

bool CudfCacheMachine::addCacheData(std::unique_ptr<ral::cache::CacheData> cache_data,
...
bool CudfCacheMachine::addToCache(std::unique_ptr<ral::frame::BlazingTable> table,
                                  std::string message_id, bool always_add,
  ...
  if (!this->something_added || table->num_rows() > 0 || always_add) {
    ...
    if (table->get_execution_backend().id() == ral::execution::backend_id::CUDF) {
      ral::frame::BlazingCudfTable* cudf_table_ptr =
          dynamic_cast<ral::frame::BlazingCudfTable*>(table.get());
    ...
    while (cacheIndex < memory_resources.size()) {
      ...
      if (memory_to_use < this->memory_resources[cacheIndex]->get_memory_limit() ||
        ...
        if (cacheIndex == 0 &&
            table->get_execution_backend().id() == ral::execution::backend_id::CUDF) {
          std::unique_ptr<ral::frame::BlazingCudfTable> cudf_table(
              dynamic_cast<ral::frame::BlazingCudfTable*>(table.release()));
          ...
          std::unique_ptr<CacheData> cache_data =
              std::make_unique<GPUCacheData>(std::move(cudf_table), metadata);
          auto item = std::make_unique<message>(std::move(cache_data), message_id);
          this->waitingCache->put(std::move(item));
...
// order) that is in the GPU put it in RAM or Disk as oppropriate this function does not
// change the order of the caches
size_t CudfCacheMachine::downgradeGPUCacheData() {
...
*/
class ConcatenatingCacheMachine : public CudfCacheMachine {
 public:
  ConcatenatingCacheMachine(std::shared_ptr<Context> context,
                            std::string cache_machine_name)
      : CudfCacheMachine(context, cache_machine_name) {}
...
std::shared_ptr<CacheMachine> make_single_machine(std::shared_ptr<Context> context, std::string cache_machine_name, bool log_timeout, int cache_level_override, bool is_array_access) {
  return std::shared_ptr<CacheMachine>(new CudfCacheMachine(
      context, cache_machine_name, log_timeout, cache_level_override, is_array_access));
}

std::shared_ptr<CacheMachine> make_concatenating_machine(std::shared_ptr<Context> context, std::size_t concat_cache_num_bytes, int num_bytes_timeout, bool concat_all, std::string cache_machine_name) {
  return std::shared_ptr<CacheMachine>(new ConcatenatingCacheMachine(context, concat_cache_num_bytes, num_bytes_timeout,
                                    concat_all, cache_machine_name));
}

REGISTER_DISPATCH(make_single_machine_stub, &ral::cache::make_single_machine)
REGISTER_DISPATCH(make_concatenating_machine_stub, &ral::cache::make_concatenating_machine)
